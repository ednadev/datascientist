...
public class JuryThread extends Thread{
	...
	public void run() {
		...
		System.out.println("comm TEST : " + comm);
		switch(comm) { //실수형은 못온다
			...
			case Command.SELLSHARES: //sellShares()
				try {
					...
					System.out.println("팔려는 주식이 수량이 너무 많아요...jury...");
					r.setStatus(-3);
			...
			case Command.GETALLSTOCK: //getAllStocks()
				...
				break;
			case Command.GETALLCUSTOMER: //getAllCustomers()
				try {
		...
		try {
			oos.writeObject(cmd);
			System.out.println("Server writeCommand....end..");
		} catch(Exception e) {
			System.out.println("Server Protocol writeCommand....error" + e);
		}
		}//while
	}//run
}//class
