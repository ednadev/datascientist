...
public class Protocol {
	...
	public Vector<SharesRec> getPortfolio(String ssn) throws RecordNotFoundException {
		...
		int status = getResponse();
		if(status==-1) throw new RecordNotFoundException("회원이 없습니다..");
		
		for(int i=0; i<cmd.getResults().size(); i++) {
			//나중에 확인
			//list.add(((CustomerRec) cmd.getResults().get(i)).getPortfolio());
	...
	public CustomerRec getCustomer(String ssn) throws RecordNotFoundException {
		CustomerRec cust = new CustomerRec();
		//1. 도시락 싼다.
		cmd = new Command(Command.GETCUSTOMER);
		String[] str = {ssn};
		cmd.setArgs(str);
	...
	public void buyShares(String ssn, String symbol, int quantity) {
		...
		String[] str = {ssn, symbol, Integer.toString(quantity)}; //quantity + ""도 가능, String.valueOf(quantity)
		cmd.setArgs(str);
	...
	public void sellShares(String ssn, String symbol, int quantity) throws InvalidTransactionException, RecordNotFoundException {
		...
		String[] str = {ssn, symbol, Integer.toString(quantity)};
		cmd.setArgs(str);
